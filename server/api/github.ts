...
class GithubAPI extends ExternalAPI {
  ...
  public async getPosterarrReleases({
    ...
    try {
      ...
      logger.warn(
        "Failed to retrieve GitHub releases. This may be an issue on GitHub's end. Posterarr can't check if it's on the latest version.",
  ...
  public async getPosterarrCommits({
    ...
    try {
      ...
      logger.warn(
        "Failed to retrieve GitHub commits. This may be an issue on GitHub's end. Posterarr can't check if it's on the latest version.",
        { label: 'GitHub API', errorMessage: e.message }
      );
      return [];
    }
